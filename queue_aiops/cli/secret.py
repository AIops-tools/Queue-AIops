"""``queue-aiops secret`` — manage the encrypted credential store.

Secrets (Monitoring API keys) are stored in ``~/.queue-aiops/secrets.enc``
...
from queue_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: queue-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update QUEUE_AIOPS_MASTER_PASSWORD.[/]")
