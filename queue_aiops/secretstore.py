"""Encrypted secret storage for queue-aiops.

Secrets (redis / rabbitmq passwords) are NEVER written to disk in plaintext. They
live in ``~/.queue-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
HMAC-SHA256). The Fernet key is derived from a master password via scrypt —
the password itself is never stored; only a random per-store salt and the
ciphertext are on disk.

The master password is resolved (in order) from:

  1. the env var ``QUEUE_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
be vendored per tool exactly like the governance harness. To adapt it for
another tool, change the three constants below.
...
APP_NAME = "queue-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "QUEUE_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
# on commodity hardware in 2026 — strong for an interactive unlock, cheap
# enough to run once per process.
_SCRYPT_N = 2**15
_SCRYPT_R = 8
_SCRYPT_P = 1
_KEY_LEN = 32
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``QUEUE_<TARGET>_SECRET``; ``prefix='QUEUE_'`` and
    ``suffix='_PASSWORD'`` map them back to a target name. The plaintext
    ``.env`` is renamed to ``.env.migrated`` (chmod 600) so nothing is silently
    lost. Returns the list of imported target names.
    """
    ...
    for line in LEGACY_ENV_FILE.read_text("utf-8").splitlines():
        ...
        if not (key.startswith(prefix) and key.endswith(suffix) and value):
            continue
