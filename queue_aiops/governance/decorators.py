"""The ``@governed_tool`` decorator — mandatory wrapper for all queue MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.queue-aiops/audit.db``
...
from queue_aiops.governance.audit import detect_agent, get_engine
from queue_aiops.governance.budget import BudgetExceeded, get_budget
from queue_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from queue_aiops.governance.patterns import PatternMatch, get_pattern_engine
from queue_aiops.governance.policy import PolicyResult, get_policy_engine
from queue_aiops.governance.readonly import is_read_only, read_only_denial
from queue_aiops.governance.sanitize import sanitize

_log = logging.getLogger("queue-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all queue MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.queue-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("QUEUE_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("QUEUE_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (QUEUE_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set QUEUE_AUDIT_APPROVED_BY to the authorizing human (and "
            f"QUEUE_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`queue_aiops.governance.outcome`). The change may be live, so an
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight queue calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``queue_aiops.ops.jobs`` → ``queue-aiops``
    ``mcp_server.server`` → ``queue-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("queue_aiops") or module.startswith("mcp_server"):
        return "queue-aiops"
    return "unknown"
