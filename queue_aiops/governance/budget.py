...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``QUEUE_RUNAWAY_MAX`` times within ``QUEUE_RUNAWAY_WINDOW_SEC`` trips a
...
from queue_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing queue-specific; the decorator wraps it into a
