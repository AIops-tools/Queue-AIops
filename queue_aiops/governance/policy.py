"""Policy engine — rule-based access control for queue MCP tools.

Rules are loaded from ``~/.queue-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("queue-aiops.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("QUEUE_POLICY_DISABLED") == "1":
        ...
        if not tiers:
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        f"No rules file at {self._path} — high/critical operations require a named approver (QUEUE_AUDIT_APPROVED_BY) by default. Author rules.yaml "
