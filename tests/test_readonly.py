...
from queue_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "delete_queue" in dropped and "purge_queue" in dropped
        assert "list_queues" not in dropped and "redis_slowlog" not in dropped
        ...
        assert "list_queues" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
