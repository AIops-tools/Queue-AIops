...
throwaway home (``QUEUE_AIOPS_HOME``) and assert that the rows compliance
...
from queue_aiops.governance import PolicyDenied, governed_tool
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("QUEUE_AUDIT_APPROVED_BY", raising=False)
    monkeypatch.delenv("QUEUE_POLICY_DISABLED", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="cache1")
...
def test_high_risk_denied_without_approver_and_denial_is_audited(gov_home):
    ...
    with pytest.raises(PolicyDenied, match="requires 'dual' approval"):
        _drop_widget(name="w2", target="cache1")
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    ...
    result = _drop_widget(name="w3", target="cache1")
    assert result["status"] == "dropped"

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert len(audit) == 1
    assert audit[0]["status"] == "ok"
    assert audit[0]["approved_by"] == "queueops-alice"
...
def test_operator_rules_file_restores_tier_none_for_high_risk(gov_home):
    ...
    _reset_singletons()
    result = _drop_widget(name="w4", target="cache1")
    assert result["status"] == "dropped"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: redis_config_set must
    capture the parameter's prior value before setting and persist the
    ...
    result = gov.redis_config_set(parameter="maxmemory-policy", value="allkeys-lru")
    assert result["priorState"] == {"value": "noeviction"}
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "redis_config_set"
    assert json.loads(undo[0]["undo_params"]) == {
        "parameter": "maxmemory-policy", "value": "noeviction",
    }

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["redis_config_set"]
